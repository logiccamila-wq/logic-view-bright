    ...
    <Card className="p-4">
      ...
      <div className="grid grid-cols-6 gap-2 mb-3">
        ...
        <VehicleSelect 
          ...
          onChange={(v) => setForm({ ...form, vehicle_plate: v })} 
          placeholder="Placa"
        />
          ...
          <div key={nc.id} className={`flex items-center justify-between p-2 rounded-md border ${nc.rpn >= 200 ? "bg-red-50" : nc.rpn >= 125 ? "bg-yellow-50" : ""}`}>
            {editingId === nc.id ? (
              <div className="flex-1 grid grid-cols-6 gap-2 items-center">
                <Input value={(editing.description as any) ?? nc.description} onChange={(e) => setEditing({ ...editing, description: e.target.value })} />
                <Input type="number" min={1} max={10} value={(editing.severity as any) ?? nc.severity} onChange={(e) => setEditing({ ...editing, severity: Number(e.target.value) })} />
                <Input type="number" min={1} max={10} value={(editing.occurrence as any) ?? nc.occurrence} onChange={(e) => setEditing({ ...editing, occurrence: Number(e.target.value) })} />
                <Input type="number" min={1} max={10} value={(editing.detection as any) ?? nc.detection} onChange={(e) => setEditing({ ...editing, detection: Number(e.target.value) })} />
                <Button onClick={() => { const payload = { ...nc, ...editing, severity: Number((editing.severity as any) ?? nc.severity), occurrence: Number((editing.occurrence as any) ?? nc.occurrence), detection: Number((editing.detection as any) ?? nc.detection) } as NC; edit(payload); setEditingId(null); setEditing({}); }}>Salvar</Button>
                <Button variant="outline" onClick={() => { setEditingId(null); setEditing({}); }}>Cancelar</Button>
              </div>
            ) : (
              <div className="flex items-center justify-between w-full">
                ...
                <div className="flex gap-2">
                  ...
                  {nc.status !== "closed" && <Button variant="outline" onClick={() => { setEditingId(nc.id); setEditing({ id: nc.id, description: nc.description, severity: nc.severity, occurrence: nc.occurrence, detection: nc.detection } as any); }}>Editar</Button>}
                </div>
              </div>
            )}
          </div>
