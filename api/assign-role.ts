export default async function handler(req: any, res: any) {
  ...
  if (!authUserId && email) {
    ...
    if (!r.ok) return res.status(r.status).json({ error: "failed_resolving_user", detail: await r.text() })
    const js = await r.json()
    const user = Array.isArray(js) ? js[0] : js?.users?.[0]
    if (!user?.id) return res.status(404).json({ error: "user_not_found" })
    authUserId = user.id
  }
  if (!authUserId) return res.status(400).json({ error: "uid or email required" })
  const ins = await fetch(`${supabaseUrl}/rest/v1/user_roles`, {
    method: "POST",
    headers: { "Content-Type": "application/json", Authorization: `Bearer ${serviceKey}`, apikey: serviceKey },
    body: JSON.stringify({ user_id: authUserId, role })
  })
  if (ins.status !== 201) return res.status(ins.status).json({ error: "insert_failed", detail: await ins.text() })
